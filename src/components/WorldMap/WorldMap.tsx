...
const WorldMap: React.FC<WorldMapProps> = ({ profile, exchangeRates }) => {
    ...
    g.selectAll('.country')
      ...
      .on('mouseout', function (event) {
        ...
        setTimeout(() => {
          ...
          if (tooltipElement && tooltipElement.matches(':hover')) return;
          setTooltip(null);
  ...
  const resetZoom = () => {
    ...
    g.selectAll('.country').attr('d', path.current as any);
  ...
  const countryNames = geoData.map((d) => d.properties.name);
  ...
  useEffect(() => {
    ...
    if (!searchQuery.trim()) {
      g.selectAll<SVGPathElement, Country>('.country').classed(
        'highlighted',
        false,
      );
    ...
    g.selectAll<SVGPathElement, Country>('.country').sort((a, b) => {
      const isA = a.properties.name.toLowerCase() === searchQuery.toLowerCase();
      const isB = b.properties.name.toLowerCase() === searchQuery.toLowerCase();
      return isA ? 1 : isB ? -1 : 0;
    });

    const found = geoData.find((d) =>
      d.properties.name.toLowerCase().includes(searchQuery.toLowerCase()),
    );

    if (found && path.current && zoomBehavior.current) {
      const [[x0, y0], [x1, y1]] = path.current.bounds(
        found as Feature<Geometry>,
      );
      const dx = x1 - x0;
      const dy = y1 - y0;
      const x = (x0 + x1) / 2;
