...
export const countries: TaxStrategies = {
  ...
  Malta: (profile, exchangeRates) => {
    ...
    return {
      ...
      notice:
        "There are types of income taxes. Also depends on marriage and kids.",
    };
  },
...
2023-2024"`,
  ...
  Israel: (profile, exchangeRates) => {
    ...
    return {
      ...
      notice:
        "Depends on marriage, kids, etc. There are types of income taxes.",
    };
  },
  ...
  Germany: (profile, exchangeRates) => {
    ...
    const taxFreeAllowance = profile.status === "married" ? 24192 : 12096;
    const taxableIncome = Math.max(0, localCurrencyIncome - taxFreeAllowance);

    if (taxableIncome > 0) {
      if (taxableIncome <= (profile.status === "married" ? 136858 : 68429)) {
        tax =
          (taxableIncome - (profile.status === "married" ? 24192 : 12096)) *
          0.14;
      } else if (
        taxableIncome <= (profile.status === "married" ? 555650 : 277825)
      ) {
        tax =
          (taxableIncome - (profile.status === "married" ? 136858 : 68430)) *
            0.42 +
          ((profile.status === "married" ? 136858 : 68430) -
            (profile.status === "married" ? 24192 : 12096)) *
            0.14;
      } else {
        tax =
          (taxableIncome - (profile.status === "married" ? 555650 : 277825)) *
            0.45 +
          ((profile.status === "married" ? 555650 : 277825) -
            (profile.status === "married" ? 136858 : 68430)) *
            0.42 +
          ((profile.status === "married" ? 136858 : 68430) -
            (profile.status === "married" ? 24192 : 12096)) *
  ...
  Australia: (profile, exchangeRates) => {
    ...
    return {
      ...
      notice:
        "There are offsets for those with taxable income up to AUD 66,667",
    };
  },
  //Resident or Non-resident
  ...
  Denmark: (profile, exchangeRates) => {
    ...
    return {
      ...
      notice:
        "Used the ordinary tax scheme by up to 52.07%. There are types of income taxess",
    };
  },
...
2023-2024"`,
  ...
  Montenegro: (profile, exchangeRates) => {
    ...
    return {
      ...
      notice:
        "There are types of income taxes: salary or business. Here is only for salary",
    };
  },
